...
function _validateUri(ret: URI, _strict?: boolean): void {
	...
	if (ret.scheme && !_schemePattern.test(ret.scheme)) {
		const illegalChar = ret.scheme.match(/[^\w\d+.-]/)?.[0];
		throw new Error(`[UriError]: Scheme contains illegal characters.${illegalChar ? ` Found '${illegalChar}'` : ''} (len:${ret.scheme.length})`);
	}
